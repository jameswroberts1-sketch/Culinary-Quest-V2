...
// Per-host invite screen – organiser and guests, backed by Firestore
...
function loadAllowThemesFromLocal() {
  try {
...
// Load hosts from model + local cache as a fallback
...
// Local “nights” cache – hostIndex -> { date, time, theme, status }
function loadNights() {
  try {
...
export function render(root, model = {}, actions = {}) {
  ...
  const viaInviteUrl = !!inviteToken;

  // Initial “loading invite” shell so the user sees *something*
  root.innerHTML = `
    <section class="menu-card">
      ...
      <section class="menu-section">
        ...
        <p class="menu-copy">
          Just a moment while we fetch the details of this game.
        </p>
      </section>
    </section>
  `;

  // Async inner function so we can await Firestore without making render() async
  (async () => {
    // ---- resolve gameId ------------------------------------------------
    let gameId =
    ...
    if (gameId) {
      try {
        ...
        console.error("[InviteScreen] readGame failed", err);
      }
    }

    // ---- derive hosts & hostIndex -------------------------------------
    let hosts = hydrateHostsLocal(model);

    if (game && Array.isArray(game.hosts) && game.hosts.length) {
      hosts = game.hosts.map((h, idx) => ({
        name: h && typeof h.name === "string" ? h.name : `Host ${idx + 1}`,
        token: h && typeof h.token === "string" ? h.token : null,
        index: typeof h.index === "number" ? h.index : idx,
        role: h && typeof h.role === "string" ? h.role : (idx === 0 ? "organiser" : "host")
      }));
    }

    if (!hosts[0]) {
      hosts[0] = { name: "Host 1", index: 0, role: "organiser" };
    }

    let hostIndex = 0;

    if (viaInviteUrl && inviteToken && game && Array.isArray(game.hosts)) {
      const idx = game.hosts.findIndex(
        (h) => h && typeof h.token === "string" && h.token === inviteToken
      );

      if (idx === -1) {
        // Unknown / expired invite link
        root.innerHTML = `
          <section class="menu-card">
            <div class="menu-hero">
              <img
            ...
            <section class="menu-section">
              ...
              <p class="menu-copy">
                Sorry – we couldn’t find a host linked to this invite. It may have
                expired or been typed incorrectly. Please check with your organiser.
              </p>
            </section>
          </section>
        `;
      ...
      hostIndex = idx;
    } else if (Number.isInteger(model.activeHostIndex)) {
      hostIndex = model.activeHostIndex;
      if (hostIndex < 0 || hostIndex >= hosts.length) hostIndex = 0;
    }

    // Organiser is always host 0 (even if they followed their own link)
    const isOrganiser = hostIndex === 0;

    const organiserNameRaw =
      (game && typeof game.organiserName === "string" && game.organiserName) ||
    ...
    // Theme toggle: prefer Firestore setup.allowThemes; organiser can fall back to local
    let allowThemes =
      !!(game &&
        game.setup &&
    ...
    if (isOrganiser && !allowThemes) {
      // organiser on their own device → fall back to local setup cache
      allowThemes = loadAllowThemesFromLocal();
    }

    // Local nights for uniqueness + prefill
